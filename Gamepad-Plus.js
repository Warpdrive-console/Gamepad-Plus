...
(function(Scratch) {
  ...
  function getFocusedId(util) {
    if (util?.target?.focusedGamepadId !== undefined) {
  ...
  class GamepadExtension {
    ...
    getStick({ STICK, AXIS }, util) {
      ...
      return roundHundredths(applyDeadzone(pad.axes[axes[AXIS]] || 0));
    }
    ...
    getTrigger({ TRIGGER }, util) {
      ...
      if (isLeft) {
        if (layout.leftTriggerButton !== null) {
          val = pad.buttons[layout.leftTriggerButton]?.value ?? 0;
        } else {
          val = (pad.axes[2] ?? -1) / 2 + 0.5;
        }
      } else {
        if (layout.rightTriggerButton !== null) {
          val = pad.buttons[layout.rightTriggerButton]?.value ?? 0;
        } else {
          val = (pad.axes[5] ?? -1) / 2 + 0.5;
        }
      }
    ...
    remapPad({ SLOT1, SLOT2 }) {
      const slot1 = parseInt(SLOT1),
        slot2 = parseInt(SLOT2);
      if (slot1 < 1 || slot1 > MAX_CONTROLLERS || slot2 < 1 || slot2 > MAX_CONTROLLERS || slot1 === slot2)
        return;
      ...
      for (const key of state.buttonStates.keys()) {
        ...
        if (parseInt(slotId) === slot1 || parseInt(slotId) === slot2)
    ...
    getActionMap() {
      ...
      for (const action of state.customActions) {
        ...
        actionArray.push({ action: action, button: button || null });
      }
